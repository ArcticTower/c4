...
//    chart->addAxis(yAxis, Qt::AlignLeft);
    ...
    QScatterSeries *dotSeries2 = new QScatterSeries();
    dotSeries2->setMarkerShape(QScatterSeries::MarkerShapeCircle); // Set marker shape
    dotSeries2->setMarkerSize(5.0); // Set marker size
    dotSeries2->setColor(Qt::blue); // Set dot color

    qreal x = 0.0; // X-coordinate of the dot
    qreal y = 3.0; // Y-coordinate of the dot
    dotSeries->append(x, y);
    dotSeries2->append(x, y);
    chart->addSeries(dotSeries);
    chart->addSeries(dotSeries2);
    chart->createDefaultAxes();
    chart->update();
}
...
void MainWindow::on_horizontalSlider_sliderMoved(int position)
{
    ...
    QScatterSeries* dotSeries2 = static_cast<QScatterSeries*>(cv->chart()->series().at(2));
    dotSeries2->remove(0);
    ...
    if (rangeFlat == 0){
        ...
        if (mode == 2){//triangle
            ...
            for(int i = 0;i<half;i++){
                sig->setPower(dot+i,power+a+QRandomGenerator::global()->bounded(-100, 101),frameN);
                a+=35;
            }
...
{
    rangeFlat = position;
    QScatterSeries* dotSeries = static_cast<QScatterSeries*>(cv->chart()->series().at(2));
    QScatterSeries* dotSeries1 = static_cast<QScatterSeries*>(cv->chart()->series().at(1));
    qreal sx = dotSeries1->at(0).x();
    dotSeries->remove(0);
    qreal newX = sx+position;
    qreal y = 0.0;
    dotSeries->append(newX, y);
    updateChart();
